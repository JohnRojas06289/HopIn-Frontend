...
const Ridedetails = () => {
  ...
  useEffect(() => {
    const fetchNames = async () => {
      const res = await fetch(
        `${BASE_URL}/users/getNames?email=${location.state.driveremail}`
      );
  ...
  useEffect(() => {
    const fetchAvatars = async () => {
      const res = await fetch(
        `${BASE_URL}/rides/results/getavatar?email=${location.state.driveremail}`
      );
      if (!res.ok) {
        setDriverAvatar("");
      } else {
  ...
  useEffect(() => {
    const fetchDriverInfo = async () => {
      const res = await fetch(
        `${BASE_URL}/users/getDriverInfo?email=${location.state.driveremail}`
      );
    ...
    if (location.state.bookedEmail[0]) {
      const fetchAvatars = async () => {
        for (const email of location.state.bookedEmail) {
          const res = await fetch(
            `${BASE_URL}/rides/results/getavatar?email=${email}`
          );
          if (!res.ok) {
  ...
  const handleClick = async () => {
    const res = await fetch(
      `${BASE_URL}/rides/results/booking/create?id=${user}&token=${accessToken}&rideId=${location.state._id}`
    );
      ...
      <div className="max-w-3xl mt-3">
        <div>
          ...
          <div className="relative">
            <time className="sm:absolute -translate-y-8 left-0 items-center justify-center text-s font-bold w-20 h-6 mb-3 sm:mb-0 text-slate-500 rounded-full">
        ...
        <div>
          ...
          <div className="flex pl-4 text-center items-center">
            <div className="text-slate-500 text-left text-lg font-semibold mt-3 mb-3">
              Total price for 1 passenger
            </div>
            <div className="text-2xl font-semibold ml-auto mr-6">
              &euro;{location.state.price}
            </div>
          </div>
          <hr className="border-8 text-slate-400"></hr>
        </div>
        <div className="text-center">
          <div className="flex pl-4 items-center">
            ...
            <img
              src={driverAvatar || Avatars}
              className="mt-3 ml-auto mr-4 mb-3 w-12 h-12 rounded-full"
              alt="Avatar"
            />
          </div>
          <div className="w-[380px] lg:w-[480px] xl:w-[580px] mr-auto ml-auto">
            ...
            <button
              className="mt-1 mb-1 flex items-center hover:bg-slate-200 w-[380px] lg:w-[480px] xl:w-[580px] h-12 rounded-xl"
              onClick={() => {
                ...
                if (phoneNumber) {
                  ...
                  alert("El conductor no ha proporcionado un número de teléfono");
                }
              }}
            >
              <svg
              ...
              <div className="text-left text-lg font-semibold text-green-400">
                Contact {driverName}
              </div>
            </button>
            <hr></hr>
            ...
            <hr></hr>
            <div className="text-left text-lg font-semibold mt-3">
              {driverInfo.vehicle_brand + driverInfo.vehicle_model}
            </div>
            <div className=" text-slate-500 text-left text-md font-semibold mb-3">
              {driverInfo.vehicle_color}
            </div>
          </div>
          <hr className="border-8 text-slate-400"></hr>
        </div>
        {location.state.bookedEmail[0] ? (
          avatarUrl.map((avatar, index) => {
            avatarNumber++;
            return (
              <div
                key={index}
                className=" ml-auto mr-auto w-[380px] lg:w-[480px] xl:w-[580px]"
              >
                <div className="flex pl-4 text-center items-center">
                  ...
                  <img
                    src={avatarUrl[avatarNumber - 1] || Avatars}
                    className="mt-3 ml-auto mr-4 mb-3 w-12 h-12 rounded-full"
                    alt="Avatar"
                  />
                </div>
        ...
        <button
          onClick={handleClick}
          className="mt-10 bg-green-400 text-white font-bold rounded-full h-12 w-44 hover:bg-green-500"
        >
          Make Reservation
        </button>
      </div>
