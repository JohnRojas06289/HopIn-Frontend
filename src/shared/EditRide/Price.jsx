...
import { useNavigate, useLocation } from 'react-router-dom'
 ...
 const Price = () => {
  ...
  const plusPrice = () => {
    setRide(prev => ({ ...prev, price : ride.price+100 }));
  }
  const minusPrice = () => {
    if(ride.price>1){
    setRide(prev => ({ ...prev, price : ride.price-100 }));}
   ...
   return (
      <>
        <div className='text-center'>
               <h1 className='text-center mt-24 font-bold text-4xl text-slate-700'>Establece tu precio por asiento</h1>
               <div className='flex mt-16 w-[500px] lg:w-[600px] xl:w-[700px] ml-auto mr-auto'>
                    ...
                    <label className=' ml-auto mr-auto text-8xl font-semibold text-slate-700'>COP {ride.price}</label>
               ...
               <br></br>
               <button className='mt-16 bg-green-400 text-white font-bold rounded-full h-12 w-28 hover:bg-green-500' onClick={handleSubmitClick}>Continuar</button>
        </div>
      </>
   )
}
