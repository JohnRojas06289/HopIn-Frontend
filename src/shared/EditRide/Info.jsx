...
import '../../pages/shake.css'
 ...
 const [error, setError] = useState(null);
  ...
  const handleSubmitClick = (e) => {
    ...
    setRide(prev => ({ ...prev, info : info }));
    if (info.length<0) {
      setError('Su información sobre el viaje debe tener al menos 10 caracteres.');
    } else {
      ...
      <>
        <div className='text-center w-full'>
            <form onSubmit={handleSubmitClick}>
               <h1 className='text-center mt-24 font-bold text-4xl text-slate-700'>¿Algo más que añadir sobre tu viaje?</h1>
               <div>
               ...
               <br></br>
               <button type='submit' className='mt-16 bg-green-400 text-white font-bold rounded-full h-12 w-28 hover:bg-green-500'>Actualizar viaje</button>
            </form>
        </div>
      </>
   )
